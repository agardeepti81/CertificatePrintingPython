...
class CertificatePrinterArguments:
  def __init__(self):
    ...
    parser.add_argument("--inFile", help='Input file path use "\\dir\\dir\\f.jpg"', default = "\\deepti\\python\\CertficatePrinting\\TechBridge.jpeg")
...
img = Image.open(cpargs.inFile)
d = ImageDraw.Draw(img)
fnt = ImageFont.truetype(cpargs.tFont,cpargs.sizeText)
fntd = ImageFont.truetype(cpargs.dFont,cpargs.sizeDate)
d.text((cpargs.xtext,cpargs.ytext),cpargs.Name(),font=fnt,fill=(cpargs.RED,cpargs.GREEN,cpargs.BLUE))
d.text((cpargs.xdate,cpargs.ydate),cpargs.date,font=fntd,fill=(cpargs.RED,cpargs.GREEN,cpargs.BLUE))
img.save(cpargs.OutputFile())
